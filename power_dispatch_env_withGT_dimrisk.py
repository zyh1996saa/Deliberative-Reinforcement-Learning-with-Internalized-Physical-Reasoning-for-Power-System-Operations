...
class EnvConfig:
    ...
    # --- reward design: dimensioned costs (¥) ---
    # reward_mode:
    #   - "weighted": keep legacy weighted sum of physical penalties
    #   - "dimensioned": convert terms into economic-like costs (¥) and sum them
    reward_mode: str = "dimensioned"  # {"weighted","dimensioned"}

    # time step duration (hours). For hourly timeseries this is 1.0.
    dt_hours: float = 1.0

    # economic coefficients (¥)
    eta_loss_yuan_per_kwh: float = 0.5     # loss cost, e.g., 0.5 ¥/kWh (paper example)
    eta_import_yuan_per_kwh: float = 0.8   # energy purchase price proxy, adjustable
    mu_switch_yuan_per_op: float = 400.0   # switching operation cost, e.g., 400 ¥/op (paper example)

    # security penalty coefficients (¥)
    # Interpreted as ¥ per (violation_unit^2 · hour)
    penalty_v_yuan_per_pu2_hour: float = 2.0e6        # voltage violation in pu
    penalty_line_yuan_per_pct2_hour: float = 5.0e2    # line overload in %
    penalty_trafo_yuan_per_unit2_hour: float = 1.0e3  # transformer balance metric squared

    # lightweight risk term (cheap to compute, no scenario PF)
    risk_enabled: bool = True
    risk_mode: str = "margin_cvar"  # {"margin_cvar","none"}
    risk_alpha: float = 0.95        # CVaR confidence
    risk_v_margin_pu: float = 0.01  # desired safety margin to voltage limits
    risk_line_margin_pct: float = 5.0  # desired safety margin to thermal limits (%)
    eta_risk_yuan_per_unit: float = 5.0e4  # ¥ per unit of risk score




    # --- reward scaling / normalization helpers ---
    # For large systems, per-element quadratic penalties can explode when summed across all elements.
    # These toggles normalize the *dimensioned* voltage/line penalties to stabilize magnitudes.
    dim_agg_v: str = "mean"    # {"mean","sum"} aggregate of viol_v^2
    dim_agg_line: str = "mean" # {"mean","sum"} aggregate of overload^2

    # Dimensioned energy-cost switches.
    # NOTE: import_kwh already includes losses in grid-supplied power (res_ext_grid),
    # so including both import_cost and loss_cost is usually a double-count unless interpreted differently.
    dim_include_import_cost: bool = True
    dim_include_loss_cost: bool = False

    # --- timeseries unit auto-detect (safety guard) ---
    # If schema is mistakenly set to MW while data is in kW, rewards can be ~1000x larger.
    # We detect abnormally large total P and auto-scale once per process.
    ts_auto_scale_enabled: bool = True
    ts_total_p_mw_high: float = 2.0e4   # threshold of total load (MW) to trigger auto-scale
    ts_auto_scale_factor: float = 1.0e-3
...
class PowerDispatchEnv(gym.Env):
    ...
    def __init__(
        ...
        *,
        ...
        # Timeseries scaling guard (auto-detect kW mistaken as MW)
        self._ts_scale: float = 1.0
        self._ts_scale_determined: bool = False
        self._ts_scale_msg_printed: bool = False
    ...
    def _apply_timeseries_loads(self, net_cal: Any, t: int) -> None:
        ...
        # --- auto scale guard (kW mistaken as MW) ---
        # This is a pragmatic safeguard: if total load is abnormally large, down-scale once per process.
        if (not self._ts_scale_determined) and bool(getattr(self.cfg, "ts_auto_scale_enabled", True)):
            total_p = float(np.sum(np.abs(p)))
            thr = float(getattr(self.cfg, "ts_total_p_mw_high", 2.0e4))
            if total_p > thr:
                self._ts_scale = float(getattr(self.cfg, "ts_auto_scale_factor", 1.0e-3))
            else:
                self._ts_scale = 1.0
            self._ts_scale_determined = True

            if (not self._ts_scale_msg_printed) and (self._ts_scale != 1.0):
                # Keep this print one-time per worker process
                self._ts_scale_msg_printed = True
                print(
                    f"[env][pid={os.getpid()}] detected large total load {total_p:.3e} MW; "
                    f"auto-applying timeseries scale={self._ts_scale}. "
                    "If your data is already in MW, disable ts_auto_scale_enabled or raise ts_total_p_mw_high."
                )

        if self._ts_scale != 1.0:
            p = p * float(self._ts_scale)
            q = q * float(self._ts_scale)
    ...
    def _solve_and_build_obs(
        ...
        *,
        ...
        # 如果 runpp 没抛异常但结果出现非有限值，也按失败处理
        if (not pf_failed) and hasattr(net_cal, "res_bus") and len(net_cal.res_bus) > 0:
            res_bus_chk = net_cal.res_bus.reindex(self._bus_ids)
            vm_chk = res_bus_chk["vm_pu"].to_numpy(dtype=float)
            if (not np.all(np.isfinite(vm_chk))):
                pf_failed = True
                info["pf_error"] = "non-finite vm_pu in res_bus"
            if (not pf_failed) and ("va_degree" in res_bus_chk.columns):
                va_chk = res_bus_chk["va_degree"].to_numpy(dtype=float)
                # va 允许全 0，但不允许 NaN/Inf
                if (not np.all(np.isfinite(va_chk))):
                    pf_failed = True
                    info["pf_error"] = "non-finite va_degree in res_bus"
        ...
        if pf_failed:
            ...
            reward_mode = str(getattr(self.cfg, "reward_mode", "weighted")).lower()
            dt_h = float(getattr(self.cfg, "dt_hours", 1.0))
            diff_ops = int(sw_stats.get("diff", 0))
            switch_cost_yuan = float(getattr(self.cfg, "mu_switch_yuan_per_op", 0.0)) * float(diff_ops)

            # pf_failed 时：不要输出 NaN（会污染 scale / normalizer / SR 标签）
            # 这里将失败代价显式记为 fail_penalty_yuan + switch_cost_yuan
            fail_pen = float(self.cfg.fail_penalty)
            total_cost_yuan = fail_pen + float(switch_cost_yuan)

            info.update(
                {
                    "reward": -float(total_cost_yuan),

                    # 失败时无法分解物理项：全部置 0（或也可以把 fail_pen 全记到某一项）
                    "loss_term": 0.0,
                    "v_term": 0.0,
                    "line_term": 0.0,
                    "trafo_term": 0.0,
                    ...
                    # switching 仍然可计入
                    "switch_term": float(switch_cost_yuan),

                    "reward_mode": str(reward_mode),
                    "total_cost_yuan": float(total_cost_yuan),
                    "dt_hours": float(dt_h),

                    # 同步补齐 cost 字段，保证训练侧取 label 时永远 finite
                    "loss_cost_yuan": 0.0,
                    "import_cost_yuan": 0.0,
                    "v_cost_yuan": 0.0,
                    "line_cost_yuan": 0.0,
                    "trafo_cost_yuan": 0.0,
                    "risk_cost_yuan": 0.0,
                    "switch_cost_yuan": float(switch_cost_yuan),
                }
            )
            return obs, info
        ...
        # legacy weighted reward (kept for backward compatibility)
        loss_term_w = float(self.cfg.w_loss * loss_mw)
        v_term_w = float(self.cfg.w_v * v_viol)
        line_term_w = float(self.cfg.w_line * line_viol)
        switch_term_w = float(self.cfg.w_switch * switch_cost)
        trafo_term_w = float(self.cfg.w_trafo_balance * trafo_bal)
        reward_weighted = -(loss_term_w + v_term_w + line_term_w + switch_term_w + trafo_term_w)

        if reward_mode == "dimensioned":
            # per-element violations (for dimensioned costs / risk)
            res_bus_ord = net_cal.res_bus.reindex(self._bus_ids)
            vm = res_bus_ord["vm_pu"].to_numpy(dtype=float)
            viol_v = np.maximum(0.0, float(self.cfg.vmin) - vm) + np.maximum(0.0, vm - float(self.cfg.vmax))

            if hasattr(net_cal, "res_line") and (len(net_cal.res_line) > 0):
                loading = net_cal.res_line["loading_percent"].to_numpy(dtype=float)
            else:
                loading = np.zeros((0,), dtype=float)
            overload = np.maximum(0.0, loading - float(self.cfg.line_loading_limit))

            # energy terms (kWh)
            loss_kwh = float(loss_mw) * 1000.0 * dt_h
            import_mw = self._calc_import_mw(net_cal)
            import_kwh = float(import_mw) * 1000.0 * dt_h

            # economic-like costs (¥)
            loss_cost = float(self.cfg.eta_loss_yuan_per_kwh) * loss_kwh
            import_cost = float(self.cfg.eta_import_yuan_per_kwh) * import_kwh

            # switching operations: prefer diff from sw_stats if available
            diff_ops = int(sw_stats.get("diff", 0))
            switch_cost_yuan = float(self.cfg.mu_switch_yuan_per_op) * float(diff_ops)

            # security costs (quadratic penalties, ¥)

            # For large grids, summing per-element penalties can yield very large episode returns.
            # We support "mean" aggregation to keep magnitudes stable while preserving gradients.
            v_s2 = 0.0
            v_n = int(viol_v.size) if isinstance(viol_v, np.ndarray) else 0
            if v_n > 0:
                if str(getattr(self.cfg, "dim_agg_v", "mean")).lower() == "sum":
                    v_s2 = float(np.sum(np.square(viol_v)))
                else:
                    v_s2 = float(np.mean(np.square(viol_v)))

            line_s2 = 0.0
            line_n = int(overload.size) if isinstance(overload, np.ndarray) else 0
            if line_n > 0:
                if str(getattr(self.cfg, "dim_agg_line", "mean")).lower() == "sum":
                    line_s2 = float(np.sum(np.square(overload)))
                else:
                    line_s2 = float(np.mean(np.square(overload)))
            v_cost = float(self.cfg.penalty_v_yuan_per_pu2_hour) * float(v_s2) * dt_h
            line_cost = float(self.cfg.penalty_line_yuan_per_pct2_hour) * float(line_s2) * dt_h
            trafo_cost = float(self.cfg.penalty_trafo_yuan_per_unit2_hour) * float(trafo_bal ** 2) * dt_h

            # lightweight risk proxy: CVaR of deficit-to-safety-margin
            risk_cost = 0.0
            risk_v = 0.0
            risk_line = 0.0
            if bool(getattr(self.cfg, "risk_enabled", False)) and str(getattr(self.cfg, "risk_mode", "none")).lower() == "margin_cvar":
                margin_v = np.minimum(vm - float(self.cfg.vmin), float(self.cfg.vmax) - vm)
                deficit_v = self._softplus(float(self.cfg.risk_v_margin_pu) - margin_v)
                risk_v = self._cvar_tail(deficit_v, alpha=float(self.cfg.risk_alpha))

                margin_line = float(self.cfg.line_loading_limit) - loading
                deficit_line = self._softplus(float(self.cfg.risk_line_margin_pct) - margin_line)
                risk_line = self._cvar_tail(deficit_line, alpha=float(self.cfg.risk_alpha))

                risk_cost = float(self.cfg.eta_risk_yuan_per_unit) * float(risk_v + risk_line) * dt_h

            total_cost_yuan = ( (import_cost if bool(getattr(self.cfg, 'dim_include_import_cost', True)) else 0.0)
                              + (loss_cost if bool(getattr(self.cfg, 'dim_include_loss_cost', False)) else 0.0)
                              + switch_cost_yuan + v_cost + line_cost + trafo_cost + risk_cost )
            reward = -float(total_cost_yuan)

            # map dimensioned costs to legacy names for logging compatibility
            loss_term = float(loss_cost)
            v_term = float(v_cost)
            line_term = float(line_cost)
            switch_term = float(switch_cost_yuan)
            trafo_term = float(trafo_cost)
            risk_term = float(risk_cost)
        else:
            # fallback: original weighted reward
            reward = float(reward_weighted)
            loss_term = float(loss_term_w)
            v_term = float(v_term_w)
            line_term = float(line_term_w)
            switch_term = float(switch_term_w)
            trafo_term = float(trafo_term_w)
            risk_term = 0.0
            import_mw = 0.0
            loss_kwh = float(loss_mw) * 1000.0 * dt_h
            import_kwh = 0.0
        ...
        info.update(
            {
                ...
                "total_load_mw": float(net_cal.load["p_mw"].sum()) if hasattr(net_cal, "load") else float("nan"),
                "ts_scale": float(getattr(self, "_ts_scale", 1.0)),
                "import_kwh": float(import_kwh),
                "loss_cost_yuan": float(loss_cost),
                "import_cost_yuan": float(import_cost),
                "switch_cost_yuan": float(switch_cost_yuan),
                "v_cost_yuan": float(v_cost),
                "v_s2": float(v_s2),
                "v_n": int(v_n),
                "line_cost_yuan": float(line_cost),
                "line_s2": float(line_s2),
                "line_n": int(line_n),
                "trafo_cost_yuan": float(trafo_cost),
                "risk_v": float(risk_v),
                "risk_line": float(risk_line),
    ...
    def _build_obs(self, net_cal: Any, t: int, topology_id: int) -> Dict[str, np.ndarray]:
        ...
        vm = res_bus["vm_pu"].to_numpy(dtype=float)
        vm = self._nan_to_num(vm, nan=0.0, posinf=0.0, neginf=0.0)

        # va_degree 可能缺失或为 NaN：做保护
        if "va_degree" in res_bus.columns:
            va = res_bus["va_degree"].to_numpy(dtype=float)
        else:
            va = np.zeros_like(vm, dtype=float)
        va = self._nan_to_num(va, nan=0.0, posinf=0.0, neginf=0.0)

        load_p = net_cal.load["p_mw"].to_numpy(dtype=float)
        load_q = net_cal.load["q_mvar"].to_numpy(dtype=float)
        load_p = self._nan_to_num(load_p, nan=0.0, posinf=0.0, neginf=0.0)
        load_q = self._nan_to_num(load_q, nan=0.0, posinf=0.0, neginf=0.0)

        fp, fq = self.ts.get_forecast_pq(t, Hh, self._load_gisids)
        fp = self._nan_to_num(fp, nan=0.0, posinf=0.0, neginf=0.0)
        fq = self._nan_to_num(fq, nan=0.0, posinf=0.0, neginf=0.0)
        ...
        return {
            "bus_vm_pu": vm.astype(np.float32, copy=False),
            "bus_va_deg": va.astype(np.float32, copy=False),
            "load_p_mw": load_p.astype(np.float32, copy=False),
            "load_q_mvar": load_q.astype(np.float32, copy=False),
            "forecast_p_mw": fp.astype(np.float32, copy=False),
            "forecast_q_mvar": fq.astype(np.float32, copy=False),
            "time_index": np.array([int(t)], dtype=np.int32),
            "topology_id": np.array([int(topology_id)], dtype=np.int32),
            "gt_H": gt_H.astype(np.float32, copy=False),
        }

    def _build_gt_H(self, net_cal: Any) -> np.ndarray:
        """
        H features: [load_p, load_q, gen_p, gen_q, vm, va]
        Bus order: self._bus_ids
        """
        ...
        if hasattr(net_cal, "load") and len(net_cal.load) > 0:
            for _, load in net_cal.load.iterrows():
                ...
                if i is None:
                    continue
                H[i, 0] += float(load.get("p_mw", 0.0) or 0.0)
                H[i, 1] += float(load.get("q_mvar", 0.0) or 0.0)
        ...
        if hasattr(net_cal, "gen") and len(net_cal.gen) > 0:
            for _, gen in net_cal.gen.iterrows():
                ...
                if i is None:
                    continue
                H[i, 2] += float(gen.get("p_mw", 0.0) or 0.0)
                H[i, 3] += float(gen.get("q_mvar", 0.0) or 0.0)
        ...
        if hasattr(net_cal, "res_bus") and len(net_cal.res_bus) > 0:
            ...
            H[:, 4] = vm.astype(np.float32, copy=False)
            H[:, 5] = va.astype(np.float32, copy=False)
    ...
    @staticmethod
    def _nan_to_num(arr: np.ndarray, *, nan: float = 0.0, posinf: float = 0.0, neginf: float = 0.0) -> np.ndarray:
        a = np.asarray(arr)
        if a.size == 0:
            return a.astype(np.float32, copy=False)
        return np.nan_to_num(a, nan=nan, posinf=posinf, neginf=neginf).astype(np.float32, copy=False)

    @staticmethod
    def _cvar_tail(x: np.ndarray, alpha: float = 0.95) -> float:
        """
        CVaR (upper-tail) for non-negative scores.
        NaN/Inf are ignored.
        """
    ...
    @staticmethod
    def _calc_import_mw(net_cal: Any) -> float:
        """
        Proxy of power purchased from upstream grid.
        Uses res_ext_grid.p_mw if available (pandapower convention: ext_grid injects +P into network).
        """
        if hasattr(net_cal, "res_ext_grid") and (len(net_cal.res_ext_grid) > 0):
            p = net_cal.res_ext_grid["p_mw"].to_numpy(dtype=float)
            return float(np.sum(np.maximum(0.0, p)))
        return 0.0
